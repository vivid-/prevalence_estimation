...
annot.var <- function(df,databases,alle.frq.coln,annotation.coln){
  ...
  # include all rare missense variants
  missense.idx <- grep("missense",df.filtered[,annotat.idx])
  patheo.idx.1 <- c(nonss.idx,splc.acp.idx,splc.dn.idx,frms.idx,missense.idx) # indices in the idc.filtered
  ...
  patheo.idc <- unique(c(idx.left[annot.idx],idc.filtered[patheo.idx.1]))
