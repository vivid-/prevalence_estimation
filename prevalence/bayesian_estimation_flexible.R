...
type_file <- args[9] # specify the file containing the annotation/category information
chr <- args[10] # specify the chromosome number for the gene of interest
# read the input file
df <- read.table(input.file,header=T,sep="\t") 

type_df <- read.table(type_file,header=T,stringsAsFactors=F,sep="\t")
# extrat the chromosome of interest
type_df <- type_df[which(type_df$CHROM==as.numeric(chr)),]
...
# match type annotations with pathogenic variants
all_df_search <- paste0(all.df$POS,"_",all.df$REF,"_",all.df$ALT)
patho_type <- type_df[match(all_df_search,paste0(type_df$POS,"_",type_df$REF,"_",type_df$ALT)),]$type
all.df$type <- patho_type

types <- unique(all.df$type)
all.df_na <- all.df[which(is.na(all.df$type)),]
all.df_nonNa <- all.df[which(!is.na(all.df$type)),]
...
for(i in 1:length(types)){
  ...
  ind.tmp <- which(all.df$type==types[i])
...
af_changed[which(is.na(all.df$type))] <- AC_interested[which(is.na(all.df$type))]/AN_interested[which(is.na(all.df$type))]
post_params <- get_posterior_w_v(AC_interested[which(is.na(all.df$type))],AN_interested[which(is.na(all.df$type))]/2,0,0)
posterior_param[which(is.na(all.df$type)),1] <- post_params[[1]]
posterior_param[which(is.na(all.df$type)),2] <- post_params[[2]]
...
# output the result
output_file <- args[12] # destination for output
sink(output_file)
