...
filter.variant <- function(df,databases,alle.frq.coln){
  ...
  idc.common = c(idc.common,which(is.na(df[,idx.alle.frq]))) #%%%%%%%%%%%%%%%
...
dat_1 <- cbind(dat,annotation)
dat_1 <- cbind(dat_1,allele.freq)

#### extract patheogenic variants ########
databases = c("sig.emory","sig.clinvar")
#frq.coln = "AF"
frq.coln = "allele.freq"
#annotation.coln = "Annotation"
annotation.coln = "annotation"
patheo.info <- annot.var(dat_1,databases = databases,alle.frq.coln = frq.coln,annotation.coln = annotation.coln)
#result <- cbind(dat.noNa,patheo.info)
result <- cbind(dat_1,patheo.info)
